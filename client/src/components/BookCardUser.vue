...
<template>
  <div class="card bg-white shadow" style="height: 30rem">
    ...
    <div class="card-footer bg-transparent text-center">
      <button
        class="btn rounded-pill card-link fw-semibold px-3 shadow-sm"
        v-show="!requested"
        @click="requestBook"
      >
        Request
      ...
      <button
        class="btn btn-req rounded-pill card-link fw-semibold px-4 shadow-sm"
        v-show="requested"
        disabled
      >
        Requested
...
export default {
  ...
  methods: {
    ...
    requestBook(event) {
      ...
      fetch(store.getters.BASEURL + `/request/${this.book.id}`, {
        method: "POST",
        headers: {
